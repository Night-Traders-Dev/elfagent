...
class SearchHelper:
    ...
    def build_queries(
        ...
        if any(token in lowered for token in ["latest", "news", "today", "recent", "current"]):
            queries.extend([
                f"{base} official update",
                f"{base} timeline",
            ])
        elif any(token in lowered for token in ["docs", "documentation", "api", "sdk", "reference"]):
            queries.extend([
                f"{base} official docs",
                f"{base} reference",
            ])
        elif any(token in lowered for token in ["compare", "comparison", "versus", " vs "]):
            queries.extend([
                f"{base} benchmark",
                f"{base} differences",
            ])
    ...
    def compress_results(self, query: str, results, limit: int = 5):
    ...
    def merge_evidence(self, query: str, existing: list[dict], new_items: list[dict], limit: int = 6) -> list[dict]:
        merged = list(existing) + list(new_items)
        reranked = self.reranker.rerank(query, merged)
        reranked = self.embedding_reranker.rerank(query, reranked)
