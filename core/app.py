...
async def build_agent(llm_model: str | None = None):
...
async def run_turn_rich(agent, user_msg, ddg_spec):
    ...
    try:
        ...
        agent_for_turn, _, _, _ = await build_agent(selected_model)
        handler = agent_for_turn.run(user_msg=agent_input)
...
async def main():
    ...
    while True:
        ...
        if not user_msg:
            continue
        local = handle_local_command(user_msg, console, tools)
        if local == "quit":
            break
        if local == "handled":
            continue
        await run_turn_rich(agent, user_msg, ddg_spec)
